class GLManager {
  ...
  addArrayBufferData = (vertices) => this.gl.bufferData(this.gl.ARRAY_BUFFER,
                                      new Float32Array(vertices), this.gl.STATIC_DRAW);

  bindElementArrayBuffer = (buffer) => this.gl.bindBuffer(this.gl.ARRAY_BUFFER, buffer);
  unbindElementArrayBuffer = () => this.gl.bindBuffer(this.gl.ARRAY_BUFFER, null);
  addElementArrayBufferData = (vertices) => this.gl.bufferData(this.gl.ARRAY_BUFFER,
                                        new Uint16Array(vertices), this.gl.STATIC_DRAW);
  ...
  getAttribLocation = (program, attribute) => this.gl.getAttribuLocation(program, attribute);
  ...
  drawTriangles = (numIndices) => this.gl.drawElements(this.gl.TRIANGLES, numIndices, this.gl.UNSIGNED_SHORT, 0);
}
